...
class User extends UserIdentity
{
    ...
    public static function tableName()
    {
        if(!Yii::$app->arteam->auth) {
            return Yii::$app->arteam->user_table;
        }
        return '';
    }
    ...
    public static function assignRole($userId, $roleName)
    {
        try {
            Yii::$app->db->createCommand()
                    ->insert(Yii::$app->arteam->auth_assignment_table, [
                        'user_id' => $userId,
                        'item_name' => $roleName,
                        'created_at' => time(),
                    ])->execute();
    ...
    public static function revokeRole($userId, $roleName)
    {
        $result = Yii::$app->db->createCommand()
                        ->delete(Yii::$app->arteam->auth_assignment_table, ['user_id' => $userId, 'item_name' => $roleName])
                        ->execute() > 0;
    ...
    {
        return array(
            self::STATUS_ACTIVE => Yii::t('arteam', 'Active'),
            self::STATUS_INACTIVE => Yii::t('arteam', 'Inactive'),
            self::STATUS_BANNED => Yii::t('arteam', 'Banned'),
        );
    }
    ...
    public static function getGenderList()
    {
        return array(
            ...
            self::GENDER_MALE => Yii::t('arteam', 'Male'),
            self::GENDER_FEMALE => Yii::t('arteam', 'Female'),
        );
    }
    ...
    public function validateEmailUnique()
    {
        if ($this->email) {
            ...
            if ($exists AND $exists->id != $this->id) {
                $this->addError('email', Yii::t('arteam', 'This e-mail already exists'));
            }
        }
    }
        ...
        if ($this->bind_to_ip) {
            ...
            foreach ($ips as $ip) {
                if (!filter_var(trim($ip), FILTER_VALIDATE_IP)) {
                    $this->addError('bind_to_ip', Yii::t('arteam', "Wrong format. Enter valid IPs separated by comma"));
                }
            }
        }
    }
    ...
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('arteam', 'ID'),
            'username' => Yii::t('arteam', 'Login'),
            'superadmin' => Yii::t('arteam', 'Superadmin'),
            'confirmation_token' => Yii::t('arteam', 'Confirmation Token'),
            'registration_ip' => Yii::t('arteam', 'Registration IP'),
            'bind_to_ip' => Yii::t('arteam', 'Bind to IP'),
            'status' => Yii::t('arteam', 'Status'),
            'gridRoleSearch' => Yii::t('arteam', 'Roles'),
            'created_at' => Yii::t('arteam', 'Created'),
            'updated_at' => Yii::t('arteam', 'Updated'),
            'password' => Yii::t('arteam', 'Password'),
            'repeat_password' => Yii::t('arteam', 'Repeat password'),
            'email_confirmed' => Yii::t('arteam', 'E-mail confirmed'),
            'email' => Yii::t('arteam', 'E-mail'),
            'first_name' => Yii::t('arteam', 'First Name'),
            'last_name' => Yii::t('arteam', 'Last Name'),
            'skype' => Yii::t('arteam', 'Skype'),
            'phone' => Yii::t('arteam', 'Phone'),
            'gender' => Yii::t('arteam', 'Gender'),
            'birth_day' => Yii::t('arteam', 'Birthday'),
            'birth_month' => Yii::t('arteam', 'Birth month'),
            'birth_year' => Yii::t('arteam', 'Birth year'),
            'info' => Yii::t('arteam', 'Short Info'),
        ];
    }
    ...
    public function getRoles()
    {
        return $this->hasMany(Role::className(), ['name' => 'item_name'])
                        ->viaTable(Yii::$app->arteam->auth_assignment_table, ['user_id' => 'id']);
    }
    ...
    public function beforeSave($insert)
    {
        if ($insert) {
            if (php_sapi_name() != 'cli') {
                $this->registration_ip = ArteamHelper::getRealIp();
            }
