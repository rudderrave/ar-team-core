...
abstract class AbstractItem extends ActiveRecord
{
    ...
    public static function removeChildren($parentName, $childrenNames)
    {
        ...
        foreach ($childrenNames as $childName) {
            Yii::$app->db->createCommand()
                    ->delete(Yii::$app->arteam->auth_item_child_table, ['parent' => $parentName, 'child' => $childName])
                    ->execute();
        }
    ...
    {
        return Yii::$app->arteam->auth_item_table;
    }
    ...
    public static function find()
    {
        if(!Yii::$app->arteam->auth) {
            return parent::find()->andWhere([Yii::$app->arteam->auth_item_table . '.type' => static::ITEM_TYPE]);
        }
    ...
    {
        return [
            'name' => Yii::t('arteam', 'Code'),
            'description' => Yii::t('arteam', 'Role'),
            'rule_name' => Yii::t('arteam', 'Rule'),
            'group_code' => Yii::t('arteam', 'Group'),
            'data' => Yii::t('arteam', 'Data'),
            'type' => Yii::t('arteam', 'Type'),
