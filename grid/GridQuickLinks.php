...
class GridQuickLinks extends Widget
{
    ...
    public function init()
    {
        ...
        if (!$this->defaultOptions) {
            $this->defaultOptions = [
                'all' => ['label' => Yii::t('arteam', 'All'), 'filterWhere' => []],
                'active' => ['label' => Yii::t('arteam', 'Active'), 'filterWhere' => ['status' => 1]],
                'inactive' => ['label' => Yii::t('arteam', 'Inactive'), 'filterWhere' => ['status' => 0]],
            ];
        }
        ...
        if (!$this->links) {
            ...
            foreach ($this->options as $option) {
                if (($this->showCount)) {
                    ...
                    if ((ArteamHelper::isImplemented($model, OwnerAccess::CLASSNAME)
