...
class TitleActionColumn extends \yii\grid\DataColumn
{
    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('arteam', 'View'),
                    'aria-label' => Yii::t('arteam', 'View'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return Html::a(Yii::t('arteam', 'View'), $url, $options);
            };
        }
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('arteam', 'Edit'),
                    'aria-label' => Yii::t('arteam', 'Edit'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return Html::a(Yii::t('arteam', 'Edit'), $url, $options);
            };
        }
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('arteam', 'Delete'),
                    'aria-label' => Yii::t('arteam', 'Delete'),
                    'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
                    'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);

                return Html::a(Yii::t('arteam', 'Delete'), $url, $options);
            };
        }
    }
     ...
     * @param \arteam\db\ActiveRecord $model the data model
