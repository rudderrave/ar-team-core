...
/**
 * AR-team component. Contains basic settings and functions of AR-teamCMS.
 */
class Arteam extends Component
{
    ...
    /**
     * Use for activate Rest authorization. Set other parameters for use and use module visit to
     * https://github.com/rudderrave
     * Default value false.
     *
     * @var bool
     */
    ...
    /**
     * Module name use in rest authorization.
     *
     * @var string
     */
    public $auth_module = '';
    ...
    /**
     * Register AR-teamCMS DB message translations.
     */
    protected function registerTranslations()
    {
        Yii::$app->i18n->translations['arteam*'] = [
            'class' => 'arteam\db\DbMessageSource',
    ...
    protected function initLanguageOptions()
    {
        if (empty($this->languages) || !is_array($this->languages)) {
            $this->languages[Yii::$app->language] = Yii::t('arteam', 'Default Language');
        }

        if (!in_array(Yii::$app->language, array_keys($this->languages))) {
            throw new InvalidConfigException('Invalid language settings! Default application language should be included into `arteam\AR-team::$languages` setting.');
        }
    ...
    {
        $languages = Yii::$app->arteam->languages;
    ...
    public function getSourceLanguageShortcode($language)
    {
        ...
        $languageRedirects = array_flip(Yii::$app->arteam->languageRedirects);
    ...
    {
        return '<a href="http://www.arteam-soft.com/" rel="external">AR-team CMS</a>';
    }

    /**
     * Returns a string representing the current version of the AR-team core.
     * 
     * @return string the version of AR-team core
     */
